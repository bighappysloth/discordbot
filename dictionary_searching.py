...
def getEntryRecursive_dictionary(query, target_dictionary:dict):
    ...
    if len(query)==1 and isinstance(query,list):
        try:
            return {
                'status': 'success',
                'msg': target_dictionary[query[0]]
            }
        except KeyError:
            return {
                'status': 'failure',
                'msg': f'Option "{query[0]}" not found.'
            }
    else:
        if isinstance(query,str): query = query.split('.') # split by dots if on first execution.
        try:
            if len(query)==1:
                result = getEntryRecursive_dictionary(query,target_dictionary)
            else:
                result = getEntryRecursive_dictionary(query[1:],target_dictionary[query[0]])
            return result
        except KeyError:
            return {
                'status': 'failure',
...
if __name__ == '__main__':
    ...
    p = Path('.')/r'user_settings'/r'default_settings.json' # path to default settings.
