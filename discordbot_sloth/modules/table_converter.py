...
def to_text(table, spacing=2):
    ...
    for row in table:
        print(" ".join("{:{width}}".format(cell, width=width+spacing) for cell, width in zip(row, column_widths)))
...
def to_latex(table, caption=None, size=None, alignment=None, spacing=2):
    ...
    print("\\begin{tabularx}{\\textwidth}{" + "".join("{}{{X}}".format(alignment[i] if alignment is not None else "c") for i in range(len(column_widths))) + "}")
    print("\hline")
    for row in table:
        print(" ".join("{:{width}}".format(cell, width=width+spacing) for cell, width in zip(row, column_widths)) + " \\\\")
        print("\hline")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Convert a list of lists of strings to a table")
    parser.add_argument("table", nargs="+", help="the table to be converted, as a list of lists of strings")
    ...
    parser.add_argument("--spacing", type=int, default=2, help="the minimum spacing between columns")
