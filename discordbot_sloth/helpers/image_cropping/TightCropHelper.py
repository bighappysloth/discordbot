...
print(f'Loaded: {fname}, type: {type(image)}, dim(image)={image.shape}, dim(image[0][:])={image[0][:].shape}, dim(image[:][0])={image[:][0].shape}')
...
while np.array_equal(blank1,image[i,:,:]): i = i + 1
while np.array_equal(blank2,image[:,j,:]): j = j + 1
top_left_corner = (i,j)
print(f'top_left_corner={top_left_corner}') # top left corner achieved.

i,j = image.shape[0]-1,image.shape[1]-1 # now calculate the bottom right corner.
while np.array_equal(blank1,image[i,:,:]): i = i-1
while np.array_equal(blank2,image[:,j,:]): j = j - 1
...
cropped_image = image[top_left_corner[0]:bottom_right_corner[0],top_left_corner[1]:bottom_right_corner[1],:] # success
