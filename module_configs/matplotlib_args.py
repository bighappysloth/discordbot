...
class HelperString:
    ...
    list_printer = lambda x: reduce(lambda y,z: y + '\n' + z, x)
    dict_printer = lambda x: HelperString.list_printer([f'{z}: {x[z]}' for z in x])
...
def mergeColumns(*args, spacing=4, align='left'):
    ...
    for L in M: L.reverse() #reverse all columns
...
colour_usage_string          = mergeColumns(list1,list2)
